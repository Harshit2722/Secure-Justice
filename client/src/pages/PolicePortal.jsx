...
const CRIME_COLORS = {
  Theft: '#fc87aaff',      // Pastel Purple
  Fraud: 'rgba(102, 149, 236, 1)',      // Pastel Light Blue
  Cybercrime: '#f8cb50ff', // Pastel Orange
  Violence: '#4ed48bff',   // Pastel Red
};

const CRIME_DARK_COLORS = {
  Theft: '#be1d46ff',      // Darker Purple
  Fraud: '#059aebff',      // Darker Blue
  Cybercrime: '#fc821fff', // Darker Orange
  Violence: '#17a75fff',   // Darker Red
};
...
const CustomPieTooltip = ({ active, payload }) => {
  if (active && payload && payload.length) {
    ...
    const darkColor = CRIME_DARK_COLORS[name] || '#475569';
    return (
      <div className="bg-white px-4 py-2 rounded-xl shadow-xl border border-outline-variant/10">
        <p className="text-sm font-black" style={{ color: darkColor }}>
          {name}: {value}
        </p>
      </div>
    );
...
export default function PolicePortal() {
    ...
    const fetchData = async () => {
      try {
        const [statsData, casesData] = await Promise.all([
          getPoliceStats(),
          getAllFirs({ limit: 5, status: statusFilter, crime_type: crimeFilter })
        ]);
  ...
  }, [statusFilter, crimeFilter]);


  const statusOrder = ['PENDING', 'UNDER_INVESTIGATION', 'VERIFIED', 'CLOSED'];
  const statusData = statusOrder.map(orderStatus => {
    const s = stats.statusStats.find(stat => stat._id.toUpperCase() === orderStatus);
    return {
      name: orderStatus,
      count: s ? s.count : 0
    };
                ...
                >
                  {statusData.map((entry, index) => (
                    <Cell 
                      ...
                      onClick={() => setStatusFilter(entry.name.toLowerCase())}
                      style={{ 
                        cursor: 'pointer',
                        opacity: statusFilter === entry.name.toLowerCase() || !statusFilter ? 1 : 0.4
                      }}
                    />
                  ))}
                  <LabelList 
                      ...
                      return (
                        <g>
                          ...
                          <text 
                            x={x + width / 2} 
                            y={y - 12} 
                            fill={STATUS_DARK_COLORS[statusData[index].name] || "#475569"} 
                            textAnchor="middle" 
                            fontSize={13} 
                            fontWeight="900"
                          >
              ...
              <PieChart>
                ...
                >
                  {crimeData.map((entry, index) => (
                    <Cell 
                      key={`cell-${index}`} 
                      fill={CRIME_COLORS[entry.name] || DEFAULT_COLOR} 
                      onClick={() => setCrimeFilter(entry.name)}
                      style={{ 
                        cursor: 'pointer',
                        opacity: crimeFilter === entry.name || !crimeFilter ? 1 : 0.4,
                        outline: 'none',
      ...
      <section>
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            <Link to="/active-cases" className="text-primary hover:bg-primary/10 p-2.5 rounded-xl transition-all flex items-center justify-center shrink-0" title="View All Reports">
        ...
        {(statusFilter || crimeFilter) && (
          <div className="flex flex-wrap gap-2 mb-6 animate-in fade-in slide-in-from-left-4">
            {statusFilter && (
            ...
            {crimeFilter && (
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-primary/10 text-primary border border-primary/20">
                Crime: {crimeFilter}
                <button onClick={() => setCrimeFilter('')} className="hover:text-primary-dim transition-colors">
                  <span className="material-symbols-outlined text-xs">close</span>
                </button>
              </span>
            )}
            <button 
              onClick={() => { setStatusFilter(''); setCrimeFilter(''); }}
