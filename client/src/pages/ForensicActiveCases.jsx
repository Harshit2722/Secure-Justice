...
export default function ForensicActiveCases() {
  ...
  const [activeTab, setActiveTab] = useState('all'); // 'all', 'needs_analysis', 'active'
  ...
  const fetchCases = async () => {
    ...
    try {
      ...
      if (activeTab === 'needs_analysis') {
        params.assigned_forensic = 'null';
        params.status = 'verified'; // Only verified cases need analysis
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        ...
        <div className="w-full lg:w-auto flex flex-col md:flex-row gap-4 items-center">
          ...
          <div className="w-full md:w-[450px] relative group order-1">
            <input
              type="text"
              placeholder="Search reports by ID or citizen..."
      ...
      <div className="flex flex-col gap-6">
        <div className="flex items-center gap-1.5 p-1.5 bg-surface-container-lowest border border-outline-variant/10 rounded-2xl w-fit shadow-sm">
          {[
            { id: 'all', label: 'All Cases', icon: 'apps' },
            { id: 'needs_analysis', label: 'Needs Analysis', icon: 'biotech' },
            { id: 'active', label: 'Ongoing', icon: 'query_stats' },
          ].map((tab) => (
            <button
              key={tab.id}
              onClick={() => {
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="relative">
            ...
            >
              ...
              {status.replace('_', ' ') || 'Report Status'}
